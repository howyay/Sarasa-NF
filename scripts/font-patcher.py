...
class font_patcher:
    ...
    def setup_font_names(self):
        ...
        fallbackStyle = re.match("^([^-]*).*?([^-]*(?!.*-))$", self.sourceFont.fontname).groups()[1]
        fontname = self.sourceFont.fontname
        ...
        fontname += "-" + projectNameAbbreviation
...
def setup_arguments(parser):
        ...
        # args.complete = all([
        #     args.fontawesome is True,
        #     args.fontawesomeextension is True,
        #     args.fontlinux is True,
        #     args.octicons is True,
        #     args.powersymbols is True,
        #     args.pomicons is True,
        #     args.powerline is True,
        #     args.powerlineextra is True,
        #     args.material is True,
        #     args.weather is True
        # ])

        return args

def generate(font, args, extension=".ttf"):
    if type(font) is list:
        print("test1")
    else:
        # the `PfEd-comments` flag is required for Fontforge to save '.comment' and '.fontlog'.
        if font.fullname != None:
            font.generate(args.outputdir + "/" + font.fullname + extension, flags=(str('opentype'), str('PfEd-comments')))
            print("\nGenerated: {}".format(font.fontname))
        else:
            font.generate(args.outputdir + "/" + font.cidfontname + extension, flags=(str('opentype'), str('PfEd-comments')))
            print("\nGenerated: {}".format(font.fullname))

        if args.postprocess:
            subprocess.call([args.postprocess, args.outputdir + "/" + font.fullname + extension])
            print("\nPost Processed: {}".format(font.fullname))

def handle_file(args, path):
    if os.path.splitext(path)[1] == ".ttc":
        handle_ttc(args, path)
    else:
        handle_font(args, path)

def handle_ttc(args, path):
    if not args.extension:
        sys.exit("TTC detected! Please supply an extension with --extension or -ext!")
    fonts = fontforge.fontsInFile(path)
    for fontName in fonts:
        #if "Gothic" in fontName or "UI" in fontName or "黑體" in fontName or "Mono" in fontName or "Fixed" in fontName or "Slab" in fontName:
        #if "Gothic" in fontName or "UI" in fontName or ("等距" not in fontName and "黑體" in fontName) or "Mono" in fontName or "Fixed" in fontName:
        if "Gothic" in fontName or "UI" in fontName:
            continue
        handle_font(args, '%s(%s)'%(path, fontName))

def handle_font(args, font):
    font = fontforge.open(font)
    patcher = font_patcher(args, font)
    font.sfnt_names = tuple([sfnt_name for sfnt_name in list(font.sfnt_names) if "Chinese" not in sfnt_name[0] or "Japanese" not in sfnt_name[0]])
    if not args.extension:
        generate(patcher.patch(), args, "." + os.path.splitext(args.font)[1])
    else:
        generate(patcher.patch(), args, "." + args.extension)
    font.close()
...
def main():
    ...
    args = setup_arguments(parser)
    
    if os.path.isdir(args.font):
        for file in scandir(args.font):
            if file.is_file():
                handle_file(args, file.path)
    else:
        handle_file(args, args.font)
